use crate::ast::{DataType, ProgramUnitType};
use crate::codegen::{
...
fn emit_api_symbol(loc: &SourceLoc, name: &str, syms: &SymbolTable) -> Result<Arg> {
    ...
    let name_lc = crate::util::safe_identifier(&name.to_ascii_lowercase());
...
impl CodeGen<'_> {
    pub(super) fn emit_api(&self, entry: &Entry) -> Result<String> {
